#!/usr/bin/env python3
"""
Voice Trainer with TensorFlow
- Load dataset/<label>/*.wav
- Extract MFCC features (40 coeffs, 64 frames)
- CNN classifier
- Show training accuracy/loss in GUI
"""
...
SAMPLE_RATE = 16000
N_MFCC = 40
MAX_LEN = 64   # số frame MFCC tối đa (pad/truncate)
CHANNELS = 1

class VoiceTrainer(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Voice Trainer (TensorFlow)")
    ...
    def _load_dataset(self, base_dir):
        ...
        X = np.array(X)
        Y = np.array(Y)
        self._log("Loaded dataset:", X.shape, "labels:", len(labels))
        return X, Y
    ...
    def _save_model(self):
        if self.model is None:
            messagebox.showinfo("Info", "No model trained yet")
            return
